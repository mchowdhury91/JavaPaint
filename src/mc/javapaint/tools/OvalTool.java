...
public class OvalTool extends JPTool{
	...
	public void draw(ArrayList<Point> points, Graphics2D g) {
		...
		if(height <= 0 && width <= 0){
			g2.drawOval(x2, y2, Math.abs(width), Math.abs(height));
		}else if(height <= 0){
			g2.drawOval(x, y2, Math.abs(width), Math.abs(height));
		}else if(width <= 0){
			g2.drawOval(x2, y, Math.abs(width), Math.abs(height));
		}else{
